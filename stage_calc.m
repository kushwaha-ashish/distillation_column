function stage_calc(z_F, x_D, x_B, R_by_Rm)
...
xlabel('x_A');
ylabel('y_A');
title('Number of stages for given R/Rm');
...
y_ep = m_q*x_ep + c_q;
%Feed line and q-line
plot([z_F, z_F, x_ep], [0, z_F, y_ep], '--');

Rm = (x_D - y_ep)/(y_ep - x_ep);%minimum reflux found from the function
R = R_by_Rm*Rm;%actual reflux
...
fprintf('Number of stages required for (R/Rm = %.1f) = %.2f \n', R_by_Rm, stages);
end
